...
package com.tribesman.core.uri;
...
/**
 * <p>Provides a standard implementation of a Kobocoin URI with support for the following:</p>
 *
 * <ul>
 ...
 *
 * <ul>
 * <li>{@code kobocoin:<address>}</li>
 * <li>{@code kobocoin:<address>?<name1>=<value1>&<name2>=<value2>} with multiple
 ...
 * @see <a href="https://github.com.tribesman/bips/blob/master/bip-0021.mediawiki">BIP 0021</a>
 */
public class KobocoinURI {
    /**
     * Provides logging for this class
     */
    private static final Logger log = LoggerFactory.getLogger(KobocoinURI.class);
    ...
    public static final String Kobocoin_SCHEME = "kobocoin";
    ...
    /**
     * Constructs a new KobocoinURI from the given string. Can be for any network.
     *
     * @param uri The raw URI data to be parsed (see class comments for accepted formats)
     * @throws KobocoinURIParseException if the URI is not syntactically or semantically valid.
     */
    public KobocoinURI(String uri) throws KobocoinURIParseException {
        this(null, uri);
    }

    /**
     * Constructs a new object by trying to parse the input as a valid Kobocoin URI.
     *
     ...
     *
     * @throws KobocoinURIParseException If the input fails Kobocoin URI syntax and semantic checks.
     */
    public KobocoinURI(@Nullable NetworkParameters params, String input) throws KobocoinURIParseException {
        checkNotNull(input);
        ...
        try {
            ...
            throw new KobocoinURIParseException("Bad URI syntax", e);
        }

        // URI is formed as  kobocoin:<address>?<query parameters>
        // blockchain.info generates URIs of non-BIP compliant form kobocoin://address?....
        // We support both until Ben fixes his code.
        
        // Remove the kobocoin scheme.
        // (Note: getSchemeSpecificPart() is not used as it unescapes the label and parse then fails.
        // For instance with : kobocoin:129mVqKUmJ9uwPxKJBnNdABbuaaNfho4Ha?amount=0.06&label=Tom%20%26%20Jerry
        // the & (%26) in Tom and Jerry gets interpreted as a separator and the label then gets parsed
        // as 'Tom ' instead of 'Tom & Jerry')
        String schemeSpecificPart;
        if (input.startsWith("kobocoin://")) {
            schemeSpecificPart = input.substring("kobocoin://".length());
        } else if (input.startsWith("kobocoin:")) {
            schemeSpecificPart = input.substring("kobocoin:".length());
        } else {
            throw new KobocoinURIParseException("Unsupported URI scheme: " + uri.getScheme());
        }

        // Split off the address from the rest of the query parameters.
        String[] addressSplitTokens = schemeSpecificPart.split("\\?");
        if (addressSplitTokens.length == 0)
            throw new KobocoinURIParseException("No data found after the kobocoin: prefix");
        ...
        if (addressSplitTokens.length == 1) {
            ...
            if (addressSplitTokens.length == 2) {
                ...
                throw new KobocoinURIParseException("Too many question marks in URI '" + uri + "'");
            }
        }
        ...
        if (!addressToken.isEmpty()) {
            // Attempt to parse the addressToken as a Kobocoin address for this network
            try {
                Address address = new Address(params, addressToken);
                putWithValidation(FIELD_ADDRESS, address);
            } catch (final AddressFormatException e) {
                throw new KobocoinURIParseException("Bad address", e);
            }
        }

        if (addressToken.isEmpty() && getPaymentRequestUrl() == null) {
            throw new KobocoinURIParseException("No address and no r= parameter found");
        }
    }
     ...
     */
    private void parseParameters(@Nullable NetworkParameters params, String addressToken, String[] nameValuePairTokens) throws KobocoinURIParseException {
        // Attempt to decode the rest of the tokens into a parameter map.
        for (String nameValuePairToken : nameValuePairTokens) {
            final int sepIndex = nameValuePairToken.indexOf('=');
            if (sepIndex == -1)
                throw new KobocoinURIParseException("Malformed Kobocoin URI - no separator in '" +
                        nameValuePairToken + "'");
            if (sepIndex == 0)
                throw new KobocoinURIParseException("Malformed Kobocoin URI - empty name '" +
                        nameValuePairToken + "'");
     ...
     */
    private void putWithValidation(String key, Object value) throws KobocoinURIParseException {
        if (parameterMap.containsKey(key)) {
            throw new KobocoinURIParseException(String.format("'%s' is duplicated, URI is invalid", key));
        } else {
    ...
    /**
     * The Kobocoin Address from the URI, if one was present. It's possible to have Kobocoin URI's with no address if a
    ...
    public String toString() {
        StringBuilder builder = new StringBuilder("KobocoinURI[");
        boolean first = true;
    ...
    public static String convertToKobocoinURI(Address address, BigInteger amount, String label, String message) {
        return convertToKobocoinURI(address.toString(), amount, label, message);
    }

    /**
     * Simple Kobocoin URI builder using known good fields.
     * 
     * @param address The Kobocoin address
     * @param amount The amount in nanocoins (decimal)
     * @param label A label
     * @param message A message
     * @return A String containing the Kobocoin URI
     */
    public static String convertToKobocoinURI(String address, @Nullable BigInteger amount, @Nullable String label,
        ...
        builder.append(Kobocoin_SCHEME).append(":").append(address);
        
        boolean questionMarkHasBeenOutput = false;
        
        if (amount != null) {
            builder.append(QUESTION_MARK_SEPARATOR).append(FIELD_AMOUNT).append("=");
            builder.append(Utils.kobocoinValueToPlainString(amount));
