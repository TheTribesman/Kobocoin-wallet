...
package com.tribesman.core.script;
...
import static com.tribesman.core.Utils.bytesToHexString;
 ...
 *
 * <p>Kobocoin transactions don't specify what they do directly. Instead <a href="https://en.kobocoin.it/wiki/Script">a
...
public class Script {
     ...
     * The concept of a "from address" isn't well defined in Kobocoin and you should not assume the sender of a
     ...
     * logic is defined by <a href="https://github.com.tribesman/bips/blob/master/bip-0016.mediawiki">BIP 16</a>.</p>
     *
     * <p>kobocoinj does not support creation of P2SH transactions today. The goal of P2SH is to allow short addresses
    ...
    private static void executeCheckSig(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack,
        ...
        try {
            TransactionSignature sig  = TransactionSignature.decodeFromKobocoin(sigBytes, false);
    ...
    private static int executeMultiSig(Transaction txContainingThis, int index, Script script, LinkedList<byte[]> stack,
        ...
        while (sigs.size() > 0) {
            ...
            try {
                TransactionSignature sig = TransactionSignature.decodeFromKobocoin(sigs.getFirst(), false);
                Sha256Hash hash = txContainingThis.hashForSignature(index, connectedScript, (byte) sig.sighashFlags);
    ...
    public void correctlySpends(Transaction txContainingThis, long scriptSigIndex, Script scriptPubKey,
        ...
        try {
            txContainingThis = new Transaction(txContainingThis.getParams(), txContainingThis.kobocoinSerialize());
