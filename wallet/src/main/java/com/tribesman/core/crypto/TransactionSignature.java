...
import com.tribesman.core.VerificationException;
...
/**
 * A TransactionSignature wraps an {@link com.tribesman.core.ECKey.ECDSASignature} and adds methods for handling
...
public class TransactionSignature extends ECKey.ECDSASignature {
     ...
     * and generally violates people's mental model of how Kobocoin should work, thus, non-canonical signatures are now
     ...
     * of the type used by Kobocoin we have to encode them using DER encoding, which is just a way to pack the two
     * components into a structure, and then we append a byte to the end for the sighash flags.
     */
    public byte[] encodeToKobocoin() {
        try {
     ...
     */
    public static TransactionSignature decodeFromKobocoin(byte[] bytes, boolean requireCanonical) throws VerificationException {
        // Kobocoin encoding is DER signature + sighash byte.
        ...
        // In Kobocoin, any value of the final byte is valid, but not necessarily canonical. See javadocs for
