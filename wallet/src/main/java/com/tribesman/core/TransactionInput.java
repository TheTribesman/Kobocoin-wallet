...
public class TransactionInput extends ChildMessage implements Serializable {
    ...
    protected void kobocoinSerializeToStream(OutputStream stream) throws IOException {
        outpoint.kobocoinSerialize(stream);
     ...
     * "from address" is not well defined in Kobocoin and you should not assume that senders of a transaction can
     ...
     * in nodes memory pools if the existing version is time locked. See the Contracts page on the Kobocoin wiki for
