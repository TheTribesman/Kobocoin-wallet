...
/**
 * <p>A Message is a data structure that can be serialized/deserialized using both the Kobocoin proprietary serialization
...
public abstract class Message implements Serializable {
     ...
     * @param protocolVersion Kobocoin protocol version.
    ...
    private void selfCheck(byte[] msg, int offset) {
        if (!(this instanceof VersionMessage)) {
            ...
            byte[] reserialized = kobocoinSerialize();
    ...
    // These methods handle the serialization/deserialization using the custom Kobocoin protocol.
    ...
    /**
     * Should only used by KobocoinSerializer for cached checksum
     *
     * @return the checksum
     */
    byte[] getChecksum() {
    ...
    /**
     * Should only used by KobocoinSerializer for caching checksum
     *
    ...
    /**
     * Returns a copy of the array returned by {@link Message#unsafeKobocoinSerialize()}, which is safe to mutate.
     ...
     */
    public byte[] kobocoinSerialize() {
        byte[] bytes = unsafeKobocoinSerialize();
    ...
    /**
     * Serialize this message to a byte array that conforms to the kobocoin wire protocol.
     * <br/>
     ...
     */
    public byte[] unsafeKobocoinSerialize() {
        ...
        try {
            kobocoinSerializeToStream(stream);
        } catch (IOException e) {
        ...
        if (parseRetain) {
            ...
            // which means another call to kobocoinSerialize is coming.  If we didn't recache then internal
    ...
    /**
     * Serialize this message to the provided OutputStream using the kobocoin wire format.
     *
     * @param stream
     * @throws IOException
     */
    final public void kobocoinSerialize(OutputStream stream) throws IOException {
    ...
    /**
     * Serializes this message to the provided stream. If you just want the raw bytes use kobocoinSerialize().
     */
    void kobocoinSerializeToStream(OutputStream stream) throws IOException {
        log.error("Error: {} class has not implemented kobocoinSerializeToStream method.  Generating message with no payload", getClass());
    }

    /**
     * This method is a NOP for all classes except Block and Transaction.  It is only declared in Message
     * so KobocoinSerializer can avoid 2 instanceof checks + a casting.
     */
