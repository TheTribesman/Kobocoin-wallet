...
public class FilteredBlock extends Message {
    ...
    public void kobocoinSerializeToStream(OutputStream stream) throws IOException {
        if (header.transactions == null)
            header.kobocoinSerializeToStream(stream);
        else
            header.cloneAsHeader().kobocoinSerializeToStream(stream);
        merkleTree.kobocoinSerializeToStream(stream);
    }
