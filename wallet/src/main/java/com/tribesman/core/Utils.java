...
/**
 * A collection of various utility methods that are helpful for working with the Kobocoin protocol.
 * To enable debug logging from the library, run with -Dkobocoinj.logging=true on your command line.
 */
public class Utils {
    ...
    /** The string that prefixes all text messages signed using Kobocoin keys. */
    public static final String Kobocoin_SIGNED_MESSAGE_HEADER = "Kobocoin Signed Message:\n";
    public static final byte[] Kobocoin_SIGNED_MESSAGE_HEADER_BYTES = Kobocoin_SIGNED_MESSAGE_HEADER.getBytes(Charsets.UTF_8);

    // TODO: Replace this nanocoins business with something better.

    /**
     * How many "nanocoins" there are in a Kobocoin.
     * <p/>
     * A nanocoin is the smallest unit that can be transferred using Kobocoin.
    ...
    /**
     * How many "nanocoins" there are in 0.01 Kobocoins.
     * <p/>
     * A nanocoin is the smallest unit that can be transferred using Kobocoin.
     * The term nanocoin is very misleading, though, because there are only 100 million
     * of them in a coin (whereas one would expect 1 billion).
    ...
    public static BigInteger toNanoCoins(String coins) {
        ...
        if (bigint.compareTo(NetworkParameters.MAX_MONEY) > 0)
            throw new ArithmeticException("Amount larger than the total quantity of Kobocoins possible specified.");
        return bigint;
    }
     ...
     * standard procedure in Kobocoin. The resulting hash is in big endian form.
     */
     ...
     */
    public static String kobocoinValueToFriendlyString(BigInteger value) {
     ...
     */
    public static String kobocoinValueToPlainString(BigInteger value) {
        if (value == null) {
            throw new IllegalArgumentException("Value cannot be null");
        }
                
        BigDecimal valueInKOBO = new BigDecimal(value).divide(new BigDecimal(Utils.COIN));
        return valueInKOBO.toPlainString();
    }
     ...
     *
     * <p>Kobocoin only uses this "compact" format for encoding difficulty targets, which are unsigned 256bit quantities.
     ...
     *
     * <tt><p>[24] "Kobocoin Signed Message:\n" [message.length as a varint] message</p></tt>
     */
    public static byte[] formatMessageForSigning(String message) {
        try {
            ByteArrayOutputStream bos = new ByteArrayOutputStream();
            bos.write(Kobocoin_SIGNED_MESSAGE_HEADER_BYTES.length);
            bos.write(Kobocoin_SIGNED_MESSAGE_HEADER_BYTES);
