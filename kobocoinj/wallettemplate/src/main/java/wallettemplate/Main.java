...
public class Main extends Application {
    ...
    public static WalletAppKit kobocoin;
    ...
    private void init(Stage mainWindow) throws IOException {
        ...
        // Tell kobocoinj to run event handlers on the JavaFX UI thread. This keeps things simple and means
        // we cannot forget to switch threads when adding event handlers. Unfortunately, the DownloadListener
        // we give to the app kit is currently an exception and runs on a library thread. It'll get fixed in
        // a future version.
        Threading.USER_THREAD = Platform::runLater;
        // Create the app kit. It won't do any heavyweight initialization until after we start it.
        kobocoin = new WalletAppKit(params, new File("."), APP_NAME);
        if (params == RegTestParams.get()) {
            kobocoin.connectToLocalHost();   // You should run a regtest mode kobocoind locally.
        } else if (params == MainNetParams.get()) {
            // Checkpoints are block headers that ship inside our app: for a new user, we pick the last header
            // in the checkpoints file and then download the rest from the network. It makes things much faster.
            // Checkpoint files are made using the BuildCheckpoints tool and usually we have to download the
            // last months worth or more (takes a few seconds).
            kobocoin.setCheckpoints(getClass().getResourceAsStream("checkpoints"));
        }

        // Now configure and start the appkit. This will take a second or two - we could show a temporary splash screen
        // or progress widget to keep the user engaged whilst we initialise, but we don't.
        kobocoin.setDownloadListener(controller.progressBarUpdater())
               .setBlockingStartup(false)
               .setUserAgent(APP_NAME, "1.0")
               .startAndWait();
        // Don't make the user wait for confirmations for now, as the intention is they're sending it their own money!
        kobocoin.wallet().allowSpendingUnconfirmedTransactions();
        kobocoin.peerGroup().setMaxConnections(11);
        System.out.println(kobocoin.wallet());
        controller.onKobocoinSetup();
        mainWindow.show();
    }
