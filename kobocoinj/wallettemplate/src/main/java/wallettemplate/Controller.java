...
import wallettemplate.controls.ClickableKobocoinAddress;

import java.math.BigInteger;
import java.util.Date;

import static wallettemplate.Main.kobocoin;
...
public class Controller {
    ...
    public ClickableKobocoinAddress addressControl;
    ...
    public void onKobocoinSetup() {
        kobocoin.wallet().addEventListener(new BalanceUpdater());
        addressControl.setAddress(kobocoin.wallet().getKeys().get(0).toAddress(Main.params).toString());
        refreshBalanceLabel();
    }
    ...
    public void refreshBalanceLabel() {
        final BigInteger amount = kobocoin.wallet().getBalance(Wallet.BalanceType.ESTIMATED);
        balance.setText(Utils.kobocoinValueToFriendlyString(amount));
    }
}
