...
/**
 * A command line tool for manipulating wallets and working with Kobocoin.
 */
public class WalletTool {
    ...
    private static org.kobocoin.protocols.payments.Protos.PaymentRequest paymentRequest;
    ...
    public static class Condition {
        ...
        public boolean matchKobocoins(BigInteger comparison) {
            try {
    ...
    private static void send(List<String> outputs, BigInteger fee, String lockTimeStr, boolean allowUnconfirmed) throws VerificationException {
        try {
            ...
            System.err.println("Insufficient funds: have " + Utils.kobocoinValueToFriendlyString(wallet.getBalance()));
        }
    }

    private static void sendPaymentRequest(String location, boolean verifyPki) {
        if (location.startsWith("http") || location.startsWith("kobocoin")) {
            try {
                ListenableFuture<PaymentSession> future;
                if (location.startsWith("http")) {
                    future = PaymentSession.createFromUrl(location, verifyPki);
                } else {
                    KobocoinURI paymentRequestURI = new KobocoinURI(location);
                    future = PaymentSession.createFromKobocoinUri(paymentRequestURI, verifyPki);
                }
                ...
                System.exit(1);
            } catch (KobocoinURIParseException e) {
                System.err.println("Invalid kobocoin uri: " + e.getMessage());
                System.exit(1);
            } catch (InterruptedException e) {
                // Ignore.
            } catch (ExecutionException e) {
            ...
            try {
                paymentRequest = org.kobocoin.protocols.payments.Protos.PaymentRequest.newBuilder().mergeFrom(stream).build();
            } catch(IOException e) {
    ...
    private static void wait(WaitForEnum waitFor) throws BlockStoreException {
        ...
        switch (waitFor) {
            ...
            case BALANCE:
                ...
                if (condition.matchKobocoins(wallet.getBalance(Wallet.BalanceType.ESTIMATED))) {
                    latch.countDown();
                    break;
                }
                wallet.addEventListener(new AbstractWalletEventListener() {
                    ...
                    public synchronized void onChange() {
                        ...
                        if (condition.matchKobocoins(balance)) {
                            System.out.println(Utils.kobocoinValueToFriendlyString(balance));
                            latch.countDown();
                        }
                    }
                });
    ...
    private static void deleteKey() {
        ...
        if (pubkey != null) {
            ...
            try {
                ...
                System.err.println(addr + " does not parse as a Kobocoin address of the right network parameters.");
                return;
            }
        }
        if (key == null) {
